use {
    crate::pemem::{djb2_hash, get_exports_by_name, get_loaded_module_by_hash},
    obfstr::obfstr,
...
impl Syscall {
    ...
    pub fn get_ssn_by_hash(&mut self, function_hash: u32) -> Option<u16> {
        ...
        for exports in &self.nt_exports {
            if function_hash == djb2_hash(exports.0.as_bytes()) {
    ...
    pub fn get_sorted_nt_exports() -> Vec<(String, usize)> {
        let ntdll_base = unsafe { get_loaded_module_by_hash(NTDLL_HASH).expect(obfstr!("[-] Failed to get module base")) };

        let mut nt_exports = BTreeMap::new();

        for (name, addr) in unsafe { get_exports_by_name(ntdll_base).expect(obfstr!("[-] Failed to get exports by name")) } {
